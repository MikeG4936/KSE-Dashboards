...
local function options(reserve,heli)
  return {Theme=1,HeliType=heli or 1,BattRsv=reserve,CountSrc=1,MinFlight=30,
...
local owner=first.fixture.create({x=0,y=0,w=800,h=480},ownerOptions)
...
eq("duplicate native update ignores payload",contender.options==revised,false)
...
second.fixture.seed(revised) -- simulate a changed saved model configuration
...
eq("foreground takeover loads saved model settings",b.OPT.reservePct,35)
eq("foreground takeover loads saved helicopter type",b.OPT.heliType,2)
