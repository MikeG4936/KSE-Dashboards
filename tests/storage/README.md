...
`--implementation path/to/storage.lua` accepts an equivalent module returning the `Storage` table. The driver concatenates a mock, the exact helper source and contracts into a temporary script. The mock has no host filesystem access. It models EdgeTX `io.open/read/write/close`, global `fstat` and `dir`, and global `rename`/`del` returning numeric FatFS status.
...
First-install readiness uses a read-only `dir(parent)` open, including `/`, and requires the returned iterator function. EdgeTX 2.12.4's bundled [FatFS `f_stat`](https://github.com/EdgeTX/edgetx/blob/def35ad324896b45d6607d4778536b1bc5360d20/radio/src/thirdparty/FatFs/ff.c#L4781-L4807) returns `FR_INVALID_NAME` for the root directory, even on a healthy card; Lua consequently returns nil from `fstat("/")`. Native [`dir`](https://github.com/EdgeTX/edgetx/blob/def35ad324896b45d6607d4778536b1bc5360d20/radio/src/lua/api_filesystem.cpp#L28-L84) can open the root and returns an iterator only on success. KSE neither enumerates entries nor creates a probe file; the discarded iterator's userdata closes the handle through native garbage collection. Successful open establishes directory availability, not write permission or card durability. Missing/throwing/nonfunction APIs keep first-install history unavailable; valid main/backup/temporary recovery does not depend on this directory check.

The mocks reproduce native root-stat failure and independent directory availability. First-install integration starts with no CSV/recovery files, displays zero, and saves the first qualified count. Unit contracts also cover a missing parent, failed/malformed directory calls, ROM-backed API lookup and no enumeration/probe writes.
...
- When all three names appear missing and the parent directory can be opened with `dir`, a new cache may be created. Because `fstat` merges absence and I/O errors, absence is not provable through this API alone. Final rename never overwrites an existing main/backup: an undiscovered file causes promotion failure. The implementation does not promise to preserve an unreadable orphan temporary file in that ambiguous first-creation case.
- Readback and optimistic base-content checking reduce accidental overwrites but are not atomic compare-and-swap, SD durability, or concurrent-instance ownership. A same-size external modification during a read is not detectable in general. The dashboard ownership lease separately prevents competing KSE instances from servicing this shared cache; it cannot coordinate external file writers.
...
The fixture measures 200-model histories at typical and maximum (32 KiB) file sizes, covering isolated load/save and local-counter create/background-save callbacks with RF and renderer entry points stubbed. It seeds Electric mode with the KSE counter before timing the real create callback and asserts that history loaded, a new count qualified and the save changed the file. It does not use unresolved OMP identification to suppress unrelated work. Each measured path must remain below 15,000 Lua instructions. Counts include Lua filesystem-mock overhead; they exclude native I/O time and the RF/render work needed to establish a whole-radio callback budget.
