...
local function fixture(selection)
  ...
  function t:disconnect()
    self.rssi=0;self:step(false,10);self.rssi=100
  end
...
-- Native freshness lasts two 160ms aging ticks, not the whole live interval.
-- A discharged M2 must identify from staggered voltage packets with no RPM.
local pulsed=fixture();pulsed:remove(12)
pulsed.rx.value=11.3;pulsed.cell.value=3.77
local received={}
for elapsed=0,180,10 do
  local phase=elapsed%60
  if phase==0 then received.pack=elapsed end
  if phase==30 then received.cell=elapsed end
  pulsed.rx.current=received.pack~=nil
  pulsed.cell.current=received.cell~=nil
  pulsed.rx.fresh=received.pack~=nil and math.floor(elapsed/16)-math.floor(received.pack/16)<=1
  pulsed.cell.fresh=received.cell~=nil and math.floor(elapsed/16)-math.floor(received.cell/16)<=1
  pulsed:step(true,10)
end
eq(pulsed.audit.name(),"OMP M2","staggered native voltage updates identify live M2 without RPM")
eq(pulsed.audit.OMP_AUTO.status,nil,"ordinary packet gaps do not flash sensor errors")
pulsed:noRF("voltage-only identification")

-- Current is not enough: each voltage needs its own recently observed update.
for _,age in ipairs({399,400,401}) do
  local f=fixture();f.cell.fresh=false;f:step(false,10)
  f.rx.fresh=false;f.cell.fresh=true;f:step(false,age-50)
  f.cell.fresh=false;f:step(false,50)
  eq(f.audit.OMP_AUTO.ready,age<=400,"independent observed-update boundary "..age)
  if age>400 then
    f:settle(false)
    eq(f.audit.OMP_AUTO.ready,false,"current-only readings cannot revive expired evidence")
    f.rx.fresh=true;f.cell.fresh=true;f:settle(false)
    eq(f.audit.name(),"OMP M1","new updates recover after expiry")
  end
end

-- New values are read between pulses; remembered evidence is not a voltage cache.
local changed=fixture();changed:step(false,10)
changed.rx.fresh=false;changed.cell.fresh=false;changed:pack(3)
changed:step(false,30);changed:step(false,49)
eq(changed.audit.OMP_AUTO.ready,false,"changed current ratio restarts confirmation")
changed:step(false,1)
eq(changed.audit.name(),"OMP M2","current voltage replaces earlier ratio between pulses")

-- Broken/replaced inputs clear both histories while acquisition is pending.
for _,case in ipairs({
  {"missing",function(f) f:remove(7) end,
    function(f) f.cell=f:sensor(7,"Volt",3.8,0x80FE) end},
  {"noncurrent",function(f) f.cell.current=false end,
    function(f) f.cell.current=true end},
  {"native ID",function(f) f.sensors[7].id=0x81FE end,function() end},
  {"source slot",function(f) f:remove(7);f.cell=f:sensor(41,"Volt",3.8,0x80FE) end,function() end},
}) do
  local f=fixture();f:step(false,10);case[2](f);f:step(false,10);case[3](f)
  f.rx.fresh=false;f.cell.fresh=true;f:settle(false)
  eq(f.audit.OMP_AUTO.ready,false,case[1].." cannot reuse old pack update")
  f.rx.fresh=true;f.cell.fresh=false;f:settle(false)
  eq(f.audit.name(),"OMP M1",case[1].." recovers with independent new updates")
end

-- Time discontinuities restart pending acquisition, not a confirmed connection.
for _,gap in ipairs({-100,401}) do
  local f=fixture();f:step(false,10)
  f.rx.fresh=false;f.cell.fresh=false;f:step(false,gap)
  eq(f.audit.OMP_AUTO.ready,false,"time discontinuity cannot confirm "..gap)
  f.rx.fresh=true;f.cell.fresh=true;f:settle(false)
  eq(f.audit.name(),"OMP M1","new evidence recovers from time discontinuity "..gap)
  f:pack(3);f:step(false,gap)
  eq(f.audit.name(),"OMP M1","time discontinuity cannot unlock connection "..gap)
end

local locked=fixture();locked:settle(true)
locked.audit.A.flightDeadVoiceLatched=true
locked:pack(3);locked:remove(12);locked:settle(true)
eq(locked.audit.name(),"OMP M1","changed voltage and missing RPM cannot unlock connection")
eq(locked.audit.OMP_AUTO.ready,true,"live connection retains confirmed identity")
locked:disconnect()
eq(locked.audit.OMP_AUTO.ready,false,"observed disconnect makes identity unresolved")
eq(locked.audit.name(),"OMP M1","disconnect retains last display name")
eq(locked.audit.count(),nil,"disconnect suppresses counter attribution")
locked.rx.fresh=false;locked.cell.fresh=false;locked:settle(true)
eq(locked.audit.OMP_AUTO.ready,false,"reconnect cannot reuse pre-disconnect updates")
locked.rx.fresh=true;locked:step(false,10)
locked.rx.fresh=false;locked.cell.fresh=true;locked:settle(true)
eq(locked.audit.name(),"OMP M2","reconnect identifies new class from staggered updates")
eq(locked.audit.A.flightDeadVoiceLatched,false,"new class resets old warning latch")
...
-- An observed disconnect allows a new class and invalidates count eligibility.
ra.S.rpmMax=9999;ra.S.currMax=99;ra.S.tempMax=99
__mock.timer.value=25;retained:disconnect();retained:pack(3)
retained:step(false,10)
eq(ra.OMP_AUTO.ready,false,"reconnected different ratio awaits confirmation")
...
eq(extendedRpm.audit.name(),"OMP M1","RPM metadata does not gate identification")
...
rediscover.rpm.value=0;rediscover:disconnect();rediscover:settle(true)
eq(rediscover.audit.name(),"OMP M2","rediscovered source permits new ground selection")

-- Source edits cannot unlock a connection; reconnect uses newly qualified slots.
...
moved:step(false,10)
eq(moved.audit.OMP_AUTO.ready,true,"source ID move cannot unlock a live connection")
moved:disconnect();moved:step(false,10)
eq(moved.audit.OMP_AUTO.ready,false,"reconnect with moved source requires confirmation")
...
-- No RPM or ARM input gates OMP identity, even if a matching source exists.
for _,case in ipairs({
  {"missing",function(f) f:remove(12) end},
  {"running",function(f) f.rpm.value=2400 end},
  {"negative",function(f) f.rpm.value=-1 end},
  {"stale",function(f) f.rpm.fresh=false;f.rpm.current=false end},
  ...
  {"duplicate",function(f) f:sensor(14,"RPM",0,0x010C,UNIT_RPMS) end},
}) do
  local f=fixture();case[2](f);f:settle(true)
  eq(f.audit.OMP_AUTO.ready,true,case[1].." RPM cannot block voltage identification")
  eq(f.audit.name(),"OMP M1",case[1].." RPM selects voltage-based M1")
  f:noRF(case[1].." RPM ignored")
end
...
qualification:disconnect()
qualification:step(false,10)
eq(qualification.audit.OMP_AUTO.ready,false,"disconnect requires reconfirmation")
eq(qualification.audit.count(),nil,"disconnect suppresses displayed count")
__mock.timer.value=25;qualification:step(false,49)
eq(qualification.audit.OMP_AUTO.ready,false,"reconnected voltages wait full confirmation interval")
qualification:step(false,1)
eq(qualification.audit.OMP_AUTO.ready,true,"same-class connection reconfirms")
...
interrupted.cell.current=false;interrupted:step(false,10)
interrupted.cell.current=true;interrupted:step(false,10);interrupted:step(false,49)
...
counts:disconnect();counts:pack(3);counts:settle(false)
...
counts:disconnect();counts:pack(2);counts:settle(false)
