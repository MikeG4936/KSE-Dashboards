...
- Transmitter battery native rounding/color boundaries at both physical widths, clamping, the radio range as sole authority, one-second range refresh and clock rollback, independent warning voltage, absent fill for malformed/missing/throwing radio settings, and zero/noncurrent/NaN voltage.
- Native five-bar radio signal thresholds and adjacent values; authoritative zero despite stale named sensors; missing/throwing/invalid `getRSSI()` results; unchanged link-evidence flags. Radio signal display is independent of the named telemetry fallbacks used elsewhere in the engine.
...
- Rotorflight Smart Fuel authority, positive percentage without voltage, USB-only zero versus a powered empty pack, voltage fallback, reserve mapping and absence of a second display filter; unresolved OMP cells cannot be inferred from the radio name. Confirmed M1/M2 cells and chemistry run through the real identification lifecycle in the [OMP contracts](../omp_auto/README.md).
...
- Local counter ascending/countdown threshold crossing, one count per cycle, timer reset, attach above threshold, continued counting and malformed timer handling. Counting occurs at the qualifying threshold; resetting the timer allows the next flight to count.
- An empty native options descriptor, canonical counter/fuel defaults and receiver voltage strings, reserve normalization, the 0.10 V receiver-range floating-point boundary, theme-only warning/stat preservation, selective reserve/Rx reset and helicopter-type evidence reset. OMP uses the local counter while retaining the saved FC preference for restoration in Electric mode. Applying nil engine settings restores canonical defaults, including 20% reserve and valid 6.6/8.4 V receiver-pack limits.

These are baseline contracts, not a transmitter or RF simulator. They do not validate packet ownership, MSP transactions, profile-write safety, firmware queue consumption, filesystem failure recovery, retained heap, radio scheduling, instruction budgets, layout, voice file contents. Physical haptic timing/backlog behavior still requires a radio test. MSP admission and upstream transaction ownership require the separate admission regression suite; these motor-alert freshness tests do not establish MSP behavior. Existing unsafe behavior is not asserted as a desired contract.
