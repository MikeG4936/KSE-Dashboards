...
- Admission requires current valid ARM with bit zero clear and an observed fresh update within 400 ticks, a live radio link and suitable RF host/widget state. It adds no settling delay. Missing, expired, fractional, malformed or contradictory ARM/connection inputs deny admission; a reused sensor ID cannot retain an old disarmed ARM value.
- Initial profile locks show the current admission reason, including unavailable ARM telemetry. Interrupted snapshots retry after confirmed-disarm recovery; interrupted selections do not repeat automatically. Context-only invalidation uses a neutral check-profile message.
- Three-second ARM update pulses preserve the FC counter's 150-tick settle and allow initial/post-flight replies between pulses. Every admission still rejects armed ARM between pulses. The four-second boundary, expiry while current, rollback, invalid-byte recovery, source-ID changes, link/provider/reset recovery and expired callbacks are exercised.
...
- Auto waiting preserves confirmed-disarm arming diagnostics and denies type-dependent requests. Missing/stale/invalid ARM and contradictory connection states still block diagnostics. FC-name changes before callbacks reject stale diagnostic results and every selection stage, including same-type names. Direct entries reject mismatched live names; observing a temporary mismatch invalidates the old operation even if the name returns before its ACK. Provider, queue and host replacement invalidate callbacks and restart name confirmation. Name changes preserve active upstream work, pending foreign FIFO order and transport buffers.
...
The suite reports this carryover as `LIMIT`, not a flight-safety success. It validates the accepted policy of leaving RF Tool untouched while restricting new KSE admission. It does not establish radio memory/timing, RC latency, over-the-air request attribution, firmware acceptance of writes, independence of other RF Tool consumers, or safety between observations. Historical pre-fix trace comparisons are not acceptance tests for this policy.
