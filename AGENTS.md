...
- Upgrades from native Widget Settings start from defaults; keep the native descriptor explicitly empty (`options={}`). Preserve saved menu data and flight history unless the task authorizes a schema reset. Follow the [settings contract](docs/settings-menu.md) for configuration authority and transfer.
