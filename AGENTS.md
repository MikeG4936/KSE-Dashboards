...
- Preserve normal telemetry, Smart Fuel semantics, flight instruments, counters and alerts. The selected policy stops **new KSE-owned MSP admission outside safe ground** while retaining pre-arm diagnostics, ground configuration and post-flight updates. Keep RF Tool unmodified and preserve its initialization/recovery and foreign queue ownership. Already-active requests may continue upstream fragments/retries indefinitely; this is an accepted limitation, not a zero-traffic or measured-latency guarantee. See the review's feature-preservation contract for the exact boundaries.
- Establish positive, current safety evidence before admitting each profile-operation stage. Invalidate stale callbacks and remove safely identifiable pending owned work across arming, link loss, model/provider change and reset. Preserve active upstream transactions and foreign work; do not claim per-send cancellation through the unmodified RF Tool API.
