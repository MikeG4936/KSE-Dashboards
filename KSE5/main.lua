...
local PROFILE_FLIGHT_STATS_TIMEOUT = 200 -- two-second KSE bookkeeping deadline
  ...
  if apiOk and messageAdded then
    for _, message in ipairs(operation.messages) do
      if type(message) == "table" and message.command == 14 then
        decorated = true
        -- Timeout releases KSE bookkeeping only; RF Tool retains an active
        -- request and its ordinary retry policy until it completes.
        message.errorHandler = function()
