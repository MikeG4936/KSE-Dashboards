...
- **OMP Auto:** both aircraft with full and partly used packs, actual RxBt/Volt/RPM discovery and update timing, cross-aircraft reconnects, brief in-flight dropout, separate pictures/counts and automatic identification for previously saved OMP selections. Check the actual shared model's channel mapping, throttle/collective, binding and Model Match on both helicopters.
...
- **Nitro reminder:** numeric duration editing and fresh setup, Timer 1 count-up/countdown and reset, late startup, hidden callbacks/tool suspension, speech/haptic queues and strength, missing-clip fallback, Battery Voice off and both counter choices.
...
- **Fullscreen settings:** TX16S MK3/MAX on EdgeTX 2.12.4 first, then both compact sizes; hold ENTER and release without accidental activation, stationary touch hold versus slide, immediate short EXIT while the widget root has focus, App Mode activation versus active fullscreen, native field/popup Back, page scrolling, opaque background under long errors, source/file pickers, theme preview and repeated open/close. Test model/owner/fullscreen changes during draft/save, fresh defaults, previous native/prototype settings ignored, version-2 companion export/import, SD removal/corruption/backup recovery, and shared settings with separate KSE4/KSE5 themes. Keep Timer 1 counting and alerts active through connect/arm/disconnect while editing. Measure compilation, retained callbacks and complete service/render timing with RF Tool installed.
