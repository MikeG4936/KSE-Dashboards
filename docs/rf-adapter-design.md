...
Every KSE MSP request requires valid, current ARM bit 0 disarmed with the [bounded recent-update evidence](KSE4-KSE5-optimization-review.md#arm-update-timing-contract), a live link, a ready provider and no armed host contradiction. `Gov` and `Hspd` are neither sampled nor restricted for admission: rotation does not block requests when disarm is confirmed. There is no extra 40-tick (0.4-second) settle gate. Preserve the existing FC-count post-disarm settle of 150 ticks and unrelated connection/rate intervals.
