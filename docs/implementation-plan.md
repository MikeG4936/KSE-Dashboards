...
**Admission definition:** all KSE MSP requires valid, current and fresh ARM bit 0 disarmed, a live link, a ready provider and no armed host contradiction. Do not sample or restrict `Gov`/`Hspd` for admission: rotation does not block a request when disarm is confirmed. There is no extra 40-tick (0.4-second) settle gate. Preserve the existing 150-tick FC post-disarm settle and unrelated connection/rate intervals.
