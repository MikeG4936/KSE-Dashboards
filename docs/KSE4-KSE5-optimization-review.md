...
This supersedes both the original stopped-governor/zero-headspeed write policy and the stronger per-send/per-fragment/per-retry requirements in the historical findings and acceptance language below. Invalidate stale callbacks and remove safely identifiable pending owned entries when confirmed disarm is lost. Preserve sensor identity checks, staged verification/save, timeouts, active upstream transactions, foreign queue work, and official RF Tool initialization, recovery and page activity.

An already-active KSE request may continue fragments/retries indefinitely and may still affect the FC after KSE invalidates its operation. That is an accepted upstream limitation. Admission control provides no zero-wire-traffic or per-send cancellation guarantee and establishes no measured latency improvement. No RF Tool patch or additional approval is a prerequisite for the selected scope. The [RF adapter design](rf-adapter-design.md) retains stronger integration alternatives as historical evidence; the [implementation plan](implementation-plan.md) defines revised completion gates.

Normal telemetry, Smart Fuel, flight instruments, alerts and local counters continue; governor and headspeed remain display/alert inputs. Clear the pre-arm blocker banner whenever disarm cannot be confirmed; resume fresh diagnostics, configuration and post-flight FC count reads after confirmation returns, subject to their existing intervals. Original source findings and probes remain dated evidence, not a claim that the selected policy or its tests have been implemented.
...
This supersedes the earlier continuous-`isFresh` rule wherever historical findings below mention it. It is a bound on the last **observed fresh flag**, not an exact packet timestamp: firmware can already consider that packet up to roughly 320 ms old. Delayed/lost armed telemetry can delay detecting an actual arm; existing admission-only and upstream-retry limitations still apply. A callback pause that misses updates may require the next fresh pulse after resuming. Motor-stop alert and OMP identification freshness rules are unchanged.
...
**Historical policy note:** the stopped-rotor write policy and per-send guarantees in this original finding were superseded by the confirmed-disarm admission policy above. Current admission requirements apply equally to reads and writes; governor/headspeed do not gate either.
...
| Feature | Behavior with confirmed-disarm admission of new KSE MSP |
|---|---|
...
| `ARMING BLOCKED: ...` diagnostic banner | Refresh before flight; suspend its MSP refresh while armed or disarm is unconfirmed. Clear the old banner whenever confirmed-disarm evidence is unavailable so a pre-flight blocker is not presented as current. Resume fresh diagnostics after confirmed-disarm recovery. Actual FC arming checks remain unchanged |
| Battery-profile capacities, selection, verification and save | Remain available when disarm is confirmed. Use already loaded capacities in flight; defer new configuration reads and profile-operation stages while armed or disarm is unconfirmed; an already-active request can still retry upstream. Loading/reloading KSE while armed or without valid ARM telemetry may leave configuration details unavailable until disarm is confirmed |
| Rotorflight FC flight count | Keep the last confirmed total visible during flight; refresh after confirmed-disarm recovery. This does not change the FC's own counting or persistence. Initial loading without confirmed disarm may show unavailable until a permitted read |
...
**Selected implementation:** gate each new KSE diagnostics poll, configuration request, profile continuation stage and FC-count read on valid, current ARM bit 0 disarmed with bounded recent-update evidence, a live link, a ready provider and no armed host contradiction. This implements the user's scheduling preference; it is not an upstream compatibility mandate. Clear the old arming-blocker banner whenever confirmed-disarm evidence is unavailable and resume fresh diagnostics after confirmed-disarm recovery; ordinary telemetry remains live. Governor/headspeed are not sampled for admission; rotation does not block requests when disarm is confirmed. Defer admissions when ARM is missing/invalid/stale or host state contradicts disarm. Use existing telemetry inputs to establish permission, not a fresh MSP request.
...
**Selected acceptance:** compare RF Tool alone against RF Tool+KSE under identical inputs and distinguish request ownership and admission from later transmission. During armed or unconfirmed-disarm intervals, KSE admits no new periodic 101 or other owned requests; radio-native telemetry and warnings continue. Exercise loss of confirmed disarm before each admission/continuation, plus an already-active partial request or retry. Verify stale callbacks cannot admit verification/save after invalidation, while explicitly reproducing the accepted upstream retry limitation. Also cover page changes, hidden and external hosts, stale/disagreeing ARM state, link loss/recovery, counter/type changes and confirmed disarm with rotation or missing/stale governor/headspeed. Assert the latter conditions do not block admission and there is no added 40-tick settle. Resume diagnostics and post-flight count refresh after confirmed-disarm recovery. Packet traces must attribute residual upstream activity without claiming zero in-flight traffic or an unmeasured latency gain.
...
| In-flight MSP diagnostics | Both initiate MSP 101 while armed, independently of counter choice | Selected confirmed-disarm KSE admission, rank 13; preserve official RF Tool behavior |
