...
**Implementation feasibility follow-up:** the [RF adapter design review](rf-adapter-design.md) identifies missing send/cancel/receive hooks in this pinned RF Tool release. An already-loaded external provider cannot safely adopt the full policy through its existing public API. A supported RF Tool integration change or an explicitly revised compatibility contract is required; the original ranking did not establish that this could be solved entirely inside the two dashboards.
...
| Feature | Behavior with ground-only KSE MSP |
|---|---|
...
| `ARMING BLOCKED: ...` diagnostic banner | Refresh before flight; suspend its MSP refresh while armed/rotating. Clear the old banner on entering flight so a pre-flight blocker is not presented as current. Resume fresh diagnostics after verified disarmed/stopped recovery. Actual FC arming checks remain unchanged |
| Battery-profile capacities, selection, verification and save | Remain available on the ground. Use already loaded capacities in flight; defer configuration reads and prohibit profile writes during flight. Loading/reloading KSE while airborne may leave configuration details unavailable until safe ground recovery |
...
**Implementation:** make the recurring KSE diagnostics poll a ground-only feature, using current telemetry/state evidence. This implements the user's low-latency preference; it is not an upstream compatibility mandate. Clear the old arming-blocker banner when flight begins and resume fresh diagnostics after safe-ground recovery; ordinary telemetry remains live. Prevent new KSE-originated requests and additional fragments/retries of invalidated KSE operations while armed or otherwise clearly in flight. For a helicopter, motor switch position alone is inadequate: retain protection through autorotation/coast-down and uncertain/stale safety state. Establish a stable safe-ground condition before restarting polls. Do not use a fresh MSP poll to determine whether MSP is permitted; use the existing telemetry inputs.

Apply the policy before sending, not only before queue insertion. Preserve ordinary telemetry decoding, display, alarms, and cached flight counts. Do not “solve” this by disabling all RF Tool background work, monkey-patching the global transport, clearing other consumers' requests, or automatically adding `rf2bg`. If upstream functions mix receive processing with transmission, preserve official behavior and scope the KSE-specific control to owned operations through the shared adapter. An official RF Tool setup/reconnect transaction is not evidence that KSE needs a perpetual in-flight diagnostics poll.

**Acceptance:** compare RF Tool alone against RF Tool+KSE under identical inputs and distinguish message ownership. During armed/rotating intervals, KSE adds no periodic 101 or other owned MSP transmissions; radio-native telemetry and warnings continue. Exercise arming immediately before first send, a partial request, retry, verification and save; also test page changes, hidden host, stale/disagreeing ARM state, link loss/recovery, rotor coast-down, and switching counter/type. Validate with packet traces, preserving official RF Tool activity as the upstream baseline rather than asserting that the entire radio emits zero MSP. Resume KSE diagnostics and post-flight count refresh after verified safe-ground recovery.
...
| Telemetry/RF | Compare RF Tool alone versus RF Tool+KSE and identify request ownership; no KSE-added MSP during flight under the selected ground-only policy; slow custom CRSF + MSP, dropped/truncated/delayed replies, reconnect and FC reboot; hidden host and visible RF Tool |
