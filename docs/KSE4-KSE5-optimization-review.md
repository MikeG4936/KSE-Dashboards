...
The user selected **unmodified RF Tool with confirmed-disarm admission for all new KSE-owned MSP requests**. Every request, including profile writes and continuation stages, requires valid, current and fresh ARM bit 0 disarmed, a live link, a ready provider and no armed host contradiction. `Gov` and `Hspd` are neither sampled nor restricted for MSP admission: rotation does not block requests when disarm is confirmed. There is no extra 40-tick (0.4-second) settle gate. Preserve the existing 150-tick FC-count post-disarm settle and unrelated connection/rate intervals.
